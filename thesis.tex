...
\adviser{Supervisors: \\ Joseph J. Paton \\ Adam R. Kampff}  %replace with the full name of your adviser
...
\def\thevideo{\thechapter.\arabic{video}}
...
\usepackage[style=apa,backend=biber,uniquename=false,url=false,apamaxprtauth=99]{biblatex}
...
\hyphenation{Exis-te}
\hyphenation{dis-cre-pân-cia}
\hyphenation{apre-sen-ta-dos}
\hyphenation{ob-ser-vá-mos}
\hyphenation{de-mons-tra-ram}
\hyphenation{pri-mor-di-al}
\hyphenation{re-co-nhe-ce-mos}
...
\acknowledgments{\input{chapters/acknow}}
\abstract{\input{chapters/abstract}}
\abstractport{\input{chapters/abstractPT}}
...
\pagenumbering{roman} \setcounter{page}{2} \blankpage
\includepdf[pages={1}]{chapters/TitlePage.pdf} \blankpage
...
\include{chapters/chTeleology}
\printbibliography[heading=subbibliography]
\end{refsection}

\begin{refsection}
\blankpage
\chapter{Rapid Prototyping Tools for the Study of Behaviour}
\label{ch:tools}
...
\epigraph{Let us take a couple of examples from physical culture and sports. Downhill skiing and slalom make high demands on a skier's dexterity. What is the difference between slalom and plain cross-country skiing, which does not require any particular dexterity? It is in a conglomerate of unexpected, unique complications in the external situation, in a quick succession of motor tasks that are all unlike each other.}{\textsc{Nicholai A. Bernstein}, \textit{Dexterity and its Development} (1996)}
\include{chapters/chConclusions}
\printbibliography[heading=subbibliography]
\end{refsection}
