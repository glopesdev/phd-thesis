...
\begin{figure}
\centering
\includestandalone[scale=0.95]{chapters/figuresChBehaviour/extended}
...
\begin{figure}
\centering
\includestandalone[scale=0.95]{chapters/figuresChBehaviour/extendedPerformance}
...
\begin{figure}
\centering
\includestandalone[scale=0.95]{chapters/figuresChBehaviour/postureExample}
...
\begin{figure}
\centering
\includestandalone[scale=0.95]{chapters/figuresChBehaviour/posture}
...
\begin{figure}
\centering
\includestandalone[scale=0.95]{chapters/figuresChBehaviour/jumping}
...
During the two days where the stable state of the environment was reinstated, the posture of the animals was gradually restored to pre-manipulation levels (Figure \ref{fig:postureExample}B, \ref{fig:posture}A), although in many cases this adjustment happened at a slower rate than the transition from stable to unstable. Again, this postural adaptation was independent of the presence or absence of forepaw motor cortex.
...
\begin{figure}
\centering
\includestandalone[scale=0.97]{chapters/figuresChBehaviour/random}
...
\begin{figure}
\centering
\includestandalone[scale=0.92]{chapters/figuresChBehaviour/speed}
...
\begin{figure}
\centering
\includestandalone[scale=0.97]{chapters/figuresChBehaviour/ethogram}
...
\subsubsection*{Towards a neurobiological basis of robust responses}

In order to investigate the neurophysiological correlates of these robust responses in the motor cortex, in three animals we implanted flexible surface electrode grids above the dura in one hemisphere of the intact brain (Figure \ref{fig:ecog}A, also see Methods). Each step of the obstacle course was outfitted with a load cell sensor to measure the precise timing of contact and the amount of weight placed on each limb during locomotion. The entire electrocorticography (ECoG) system was synchronized on a frame-by-frame basis with the high-speed video acquisition so we could reconstruct the detailed behaviour of the animal at any point of the physiological recordings as well as relate the continuous load profile on individual steps with different phases in the locomotion cycle (Figure \ref{fig:loadCellProfile}).

\begin{figure}
\begin{center}
\includesvg[pretex=\relscale{0.4},width=\columnwidth]{chapters/figuresChBehaviour/loadCellProfile}
\end{center}
\vspace{-5mm}
...
We first asked whether there were responses in the ECoG signal over forelimb motor cortex that were modulated by stepping behaviour. Aligning the ECoG traces to the event of stepping on a permanently stable step with the contralateral paw revealed the distinct presence of an evoked response on the anterior grid channels that was absent for the ipsilateral paw (Figure \ref{fig:ecog}B, top trace). This result is consistent with the phasic modulation of discharge in pyramidal tract neurons that has been reported in the cat \cite{Armstrong1984a,Drew1996}.

\begin{figure}
\centering
\includestandalone[scale=0.97]{chapters/figuresChBehaviour/ecog}
